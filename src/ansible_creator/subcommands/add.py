...
class Add:
    ...
    def _plugin_scaffold(self) -> None:
        ...
        if self._plugin_type == "filter":
            template_data = self._get_filter_plugin_template_data()
    ...
    def _get_filter_plugin_template_data(self) -> TemplateData:
        """Get the template data for filter plugin.

        Returns:
            TemplateData: Data required for templating the filter plugin.
        """
        return TemplateData(
